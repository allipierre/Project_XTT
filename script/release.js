...
function zipAssets () {
  ...
  const zipAssets = [{
    name: 'apex-client-extension-mac.zip',
    path: path.join(outPath, 'ACE-darwin-x64', 'ACE.app')
  }, {
    name: 'apex-client-extension-windows.zip',
    path: path.join(outPath, 'ACE-win32-ia32')
  }, {
    name: 'apex-client-extension-linux.zip',
    path: path.join(outPath, 'ACE-linux-x64')
  }]
  ...
  return Promise.all(zipAssets.map(zipAsset)).then((zipAssets) => {
    return zipAssets.concat([{
      ...
      name: 'ACESetup.exe',
      path: path.join(outPath, 'windows-installer', 'ACESetup.exe')
    }, {
      name: `ACE-${version}-full.nupkg`,
      path: path.join(outPath, 'windows-installer', `ACE-${version}-full.nupkg`)
    }])
  })
}
...
function createRelease (assets) {
  const options = {
    uri: 'https://api.github.com/repos/electron/apex-client-extension/releases',
    headers: {
